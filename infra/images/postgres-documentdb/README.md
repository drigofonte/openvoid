...
extension. This is the storage half of openvoid's app data plane — every app gets its own CNPG cluster
running this image. See `docs/plans/2026-08-29-001-feat-app-data-plane-documentdb-plan.md` (U1).

## Why openvoid builds this
...
**The trade-off:** openvoid owns base-image patching here instead of inheriting CNPG's weekly rebuild.
A scheduled CI rebuild is required, not optional.

## Build

```sh
docker build --build-arg TARGETARCH=arm64 -t openvoid/postgres-documentdb:18-0.116-0 .
```
...
```sh
./smoke-test.sh openvoid/postgres-documentdb:18-0.116-0
```
...
`docs/spikes/2026-08-29-documentdb-multitenancy.md` F4.
