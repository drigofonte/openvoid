...
MongoDB wire-protocol gateway for the app data plane. Translates MongoDB commands into SQL against a
PostgreSQL backend running the DocumentDB extension, so app code can use an ordinary MongoDB driver.
See `docs/plans/2026-08-29-001-feat-app-data-plane-documentdb-plan.md` (U2).

Upstream ships this only as an OS package and inside the all-in-one emulator container, so the image is
openvoid's to build. Ubuntu 24.04 for the same reason as `postgres-documentdb`: upstream's Tier 1 target,
...
```sh
docker build --build-arg TARGETARCH=arm64 -t openvoid/documentdb-gateway:0.116-0 .
./integration-test.sh
```
