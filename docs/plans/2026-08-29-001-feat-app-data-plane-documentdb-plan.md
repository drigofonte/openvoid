...
- NetworkPolicy is load-bearing for R2, not decoration: ingress to the gateway only from the pod
  labelled with that app ID
...
- Generate a strong per-app password at provisioning; never reuse across apps
...
- `docs/runbooks/app-data-plane.md`: provisioning, wake/hibernate, diagnosing a slow or stuck app,
  restoring from backup, rotating credentials, upgrading DocumentDB across N clusters, decommissioning
...
### Phase C: Ops surface — **the rollout gate** (U7, U8, U9)
Credentials, observability, diagnostics, backups, and a rehearsed restore. **No real app receives a
database until this phase completes** (R8).
