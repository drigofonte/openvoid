...
status: complete
plan-unit: docs/plans/2026-08-29-001-feat-app-data-plane-documentdb-plan.md
...
The proposed app data plane was "Postgres + DocumentDB + FerretDB, one schema/database per app for
isolation". Three premises in that sentence were unverified and load-bearing. This spike settles them
before any plan commits to the design.

The requirement being tested: **each openvoid app gets a MongoDB-compatible database that no other app
can read or destroy.** openvoid apps run agent-generated code on behalf of arbitrary users, so
co-tenants are untrusted by definition — isolation has to be enforced, not conventional.
...
The surviving options are a PostgreSQL cluster per app, an openvoid-owned mediation layer that never
hands apps direct credentials, or a shared instance with the gap documented and accepted. That choice
trades cost against isolation strength. It was resolved in favour of a PostgreSQL cluster per app in
`docs/plans/2026-08-29-001-feat-app-data-plane-documentdb-plan.md` (KD2), which also records the
provisioning policy and the ops-gates-rollout decision that follow from it.
